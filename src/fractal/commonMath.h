#ifndef FTESTER_COMMONMATH_H
#define FTESTER_COMMONMATH_H
...
inline glm::vec4 qSquare(const glm::vec4& q) {
    return glm::vec4(
        2 * q.w * q.x, // x
        2 * q.w * q.y, // y
        2 * q.w * q.z, // z
...
inline float Julia_distanceEstimator(glm::vec3 point, // pozycja punktu na promieniu
...
inline glm::vec3 getRayDir(glm::vec3 camDir, // kierunek kamery
...
inline float raymarch(glm::vec3 rayOrgin, // pozycja startowa promienia
...
// Shading
// Gradient SDF
inline glm::vec3 gradientSDF(glm::vec3 point, glm::vec4 C, float w) {
    float e = 0.001f;
...
// Ambient oclusion
inline float ambientOclusion(glm::vec3 point,
...
// Soft shadow
inline float softShadow(glm::vec3 pt, glm::vec3 lightPos, glm::vec4 C, float w, int maxSteps = 64) {
    ...
    for (float t = 0.1f; t < lightDist && step < maxSteps && kd > 0.001f;) {
        float d = glm::abs(Julia_distanceEstimator(pt + t * lightDir, C, w));
        if (d < 0.001f) {
            kd = 0.0f;
        }
        else {
            kd = glm::min(kd, 16.0f * d / t);
        }
        t += d;
        step++;
    }
...
inline glm::vec3 shade(glm::vec3 rayOrigin,
    glm::vec3 rayDir,
    float t,
    glm::vec4 C,
    float w,
